...
namespace socket_internal
{
	SOCKET open()
	{
		...
		HY_ASSERT(socket >= 0, "socket: last_error() = %d", last_error());
		return socket;
	}
	...
	void set_blocking(SOCKET socket, bool blocking)
	{
		...
		HY_ASSERT(err == 0, "ioctlsocket: last_error() = %d", last_error());
		HY_UNUSED(err);
#endif
	}
	...
	void set_reuse_address(SOCKET socket, bool reuse)
	{
		...
		HY_ASSERT(err == 0, "setsockopt: last_error() = %d", last_error());
		HY_UNUSED(err);
	}
...
TCPSocket::TCPSocket()
{
	HY_STATIC_ASSERT(sizeof(_data) >= sizeof(SOCKET));
...
void TCPSocket::listen(u32 max)
{
	...
	HY_ASSERT(err == 0, "listen: last_error() = %d", last_error());
	HY_UNUSED(err);
}
