...
namespace sjson
{
	static const char* next(const char* json, const char c = 0)
	{
		...
		if (c && c != *json)
		{
			HY_ASSERT(false, "Expected '%c' got '%c'", c, *json);
		}
	...
	static f64 parse_number(const char* json)
	{
		...
		HY_ASSERT(ok == 1, "Failed to parse f64: %s", array::begin(number));
		HY_UNUSED(ok);
		return val;
	}
		...
		if (*json == '"')
		{
			while (*++json)
			{
				...
				else if (*json == '\\')
				{
					...
					case 't': string += '\t'; break;
					default: HY_FATAL("Bad escape character"); break;
					}
				}
				else
				{
...
/*namespace sjson
{
	...
	void parse_verbatim(const char* json, DynamicString& string)
	{
		...
		HY_ASSERT(end, "Bad verbatim string");
