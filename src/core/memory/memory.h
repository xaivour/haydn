...
namespace memory
{
	...
	{
		HY_ASSERT(align >= 1, "Alignment must be > 1");
		HY_ASSERT(align % 2 == 0 || align == 1, "Alignment must be a power of two");
...
#define HY_NEW(allocator, T) new ((allocator).allocate(sizeof(T), alignof(T))) T

/// Calls destructor on @a ptr and deallocates memory using the
/// given @a allocator.
/// @note
/// @a allocator must be a reference to an existing allocator.
#define HY_DELETE(allocator, ptr) haydn::memory::call_destructor_and_deallocate(allocator, ptr)
