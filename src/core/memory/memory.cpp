...
// {
// 	HY_FATAL("operator new forbidden");
...
// {
// 	HY_FATAL("operator delete forbidden");
// }
...
namespace memory
{
	...
	struct HeapAllocator : public Allocator
	{
		...
		~HeapAllocator()
		{
			HY_ASSERT(_allocation_count == 0 && total_allocated() == 0
	...
	struct ScratchAllocator : public Allocator
	{
		...
		~ScratchAllocator()
		{
			HY_ASSERT(_free == _allocate, "Memory leak");
		...
		void *allocate(u32 size, u32 align)
		{
			...
			HY_ASSERT(align % 4 == 0, "Must be 4-byte aligned");
			size = ((size + 3)/4)*4;
		...
		void deallocate(void *p)
		{
			...
			Header*h = header(p);
			HY_ASSERT((h->size & 0x80000000u) == 0, "Not free");
